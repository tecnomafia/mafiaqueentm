/* Codded by @᳆⃞⃚🎓𝐓𝐄𝐂𝐍𝐎�҈҉⃝Ｍ Ａ F I Ａ🇱🇰𖤐
...
// Sql
const SewQueenDB = Raviya.DATABASE.define('mafiya WhatsappBot', {
    info: {
...
async function sewQueen () {
    ...
    setInterval(async () => { 
        ...
        while (getGMTh == 19 && getGMTm == 1) {
            var ilan = ''
            if (Raviya.LANG == 'EN') ilan = '| *✨Daily Announcements Fmafiya |\n\n'
            if (Raviya.LANG == 'SI') ilan = '| *✨Smfiyaසැප් බොට් සදහා දෛනික නිවේදන✨* |\n\n'
...
${chalk.blue.italic('📲 Try To Login WhatsApp... Please Wait...')}`);
    ...
    CdSew.on('credentials-updated', async () => {
        ...
        console.log(chalk.bgGreen('👑 ᳆⃞⃚🎓𝐓𝐄𝐂𝐍𝐎�҈҉⃝Ｍ Ａ F I Ａ🇱🇰𖤐 is' + rssewqueengg));
        ...
        if (CdSew.user.jid == one || CdSew.user.jid == two || CdSew.user.jid == three || CdSew.user.jid == four || CdSew.user.jid == five || CdSew.user.jid == six || CdSew.user.jid == seven || CdSew.user.jid == eight) {
            ...
            var rsgg = await axios.get(`${mafiya_img}`, { responseType: 'arraybuffer' })
    ...
    CdSew.on('message-new', async msg => {
        ...
        // ==================== Greetings ====================
        if (mafiya.GIFORPP == 'pp' || mafiya.GIFORPP == 'Pp' || mafiya.GIFORPP == 'PP' || mafiya.GIFORPP == 'pP' ) {
    ...
    else if (mafiya.GIFORPP == 'gif' || mafiya.GIFORPP == 'Gif' || mafiya.GIFORPP == 'GIF' || mafiya.GIFORPP == 'GIf' ) {
    if (msg.messageStubType === 32 || msg.messageStubType === 28) {
            // welcome
            var gb = await getMessage(msg.key.remoteJid, 'goodbye');
            if (gb !== false) {
                var mafiya image = await axios.get(Raviya.WLP, { responseType: 'arraybuffer' })
                await CdSew.sendMessage(msg.key.remoteJid, Buffer.from(mafiyaimage.data), MessageType.video, {mimetype: Mimetype.gif, caption: gb.message +'\n\n                 ᴘᴏᴡᴇʀᴅ ʙʏ ꜱᴇᴡ ǫᴜᴇᴇɴ'});
            }
            return;
        } else if (msg.messageStubType === 27 || msg.messageStubType === 31) {
            // goodbye
            var gb = await getMessage(msg.key.remoteJid);
            if (gb !== false) {
            var mafiayimage = await axios.get(mafiay.GDB, { responseType: 'arraybuffer' })
     ...
     else if (mafiya.GIFORPP == 'img' || mafiya.GIFORPP == 'Img' || mafiya.GIFORPP == 'IMG' ||mafiya.GIFORPP == 'image' ) {
    if (msg.messageStubType === 32 || msg.messageStubType === 28) {
            // welcome
            var gb = await getMessage(msg.key.remoteJid, 'goodbye');
            if (gb !== false) {
                        ...
                        try {
                            ...
                            if (Raviya.LANG == 'SI') {
                                if (error.message.includes('URL')) {
                                    return await CdSew.sendMessage(CdSew.user.jid, '*🧟‍♂️ දෝෂ විශ්ලේෂණය [mafiya] 🧟‍♀️*' + 
                                        '\n========== ```දෝෂ නිරාකරණය කර ඇත!``` ==========' +
                                    ...
                                    return await CdSew.sendMessage(CdSew.user.jid, '*🧟‍♂️ දෝෂ විශ්ලේෂණය [mafiay] 🧟‍♀️*' + 
                                        '\n========== ```දෝෂ නිරාකරණය කර ඇත!``` ==========' +
                                ...
                                if (error.message.includes('URL')) {
                                    return await CdSew.sendMessage(CdSew.user.jid, '*🧟‍♂️ ERROR ANALYSIS [mafiya] 🧟‍♀️*' + 
                                        '\n========== ```Error Resolved!``` ==========' +
